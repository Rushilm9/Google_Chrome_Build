...
async def upload_and_review_files(
    ...
    papers_with_reviews = []
    for r in results:
        if not r.get("paper_id") or not r.get("analysis_id"):
            continue
        paper = db.query(Paper).filter(Paper.paper_id == r["paper_id"]).first()
        analysis = db.query(PaperAnalysis).filter(PaperAnalysis.analysis_id == r["analysis_id"]).first()
        if not paper or not analysis:
            continue
        file_path = getattr(paper, "file_path", None)
        ext = os.path.splitext(file_path)[1].lstrip(".").upper() if file_path else None
        papers_with_reviews.append({
            ...
            "analysis": {
                "analysis_id": analysis.analysis_id,
                "created_at": analysis.created_at,  # will be encoded safely
                "summary_text": analysis.summary_text,
                "strengths": _deserialize_list_from_db(analysis.strengths),
                "weaknesses": _deserialize_list_from_db(analysis.weaknesses),
                "gaps": _deserialize_list_from_db(analysis.gaps),
                "semantic_patterns": _deserialize_list_from_db(analysis.semantic_patterns),
            },
        })
    ...
    return JSONResponse(
        content=jsonable_encoder({
            "project_id": project_id,
            "count": len(papers_with_reviews),
            "papers_with_reviews": papers_with_reviews,
            "message": "✅ LLM literature review completed and stored successfully.",
        })
    )
...
# ---------------------------
# NEW: Save manually analyzed literature into DB (local analyses)
# ---------------------------
@router.post("/save-local-analyses", summary="Save manually analyzed literature into DB")
def save_local_analyses(payload: Dict[str, Any], db: Session = Depends(get_db)):
    """
    Accepts data in the same format as the frontend localAnalyzedPapers and stores
    each entry as a Paper + PaperAnalysis record.
    Returns the same format as /literature/literature-review-fetch for frontend.
    """

    # Support both {"localAnalyzedPapers": [...]} and [...] directly
    items = payload.get("localAnalyzedPapers") if isinstance(payload, dict) else None
    if not items and isinstance(payload, list):
        items = payload
    if not items or not isinstance(items, list):
        raise HTTPException(status_code=400, detail="Expected a JSON array under key 'localAnalyzedPapers'.")
    ...
    for item in items:
        try:
            ...
            if isinstance(raw_analysis, str):
                m = re.search(r"\{[\s\S]*\}", raw_analysis)
            ...
            gaps = parsed.get("Research Gaps") or []
            keywords = parsed.get("Keywords") or []

            # Save Paper
            paper = Paper(
                query_id=project_id,
                title=meta.get("Possible Title") or name,
                file_path=None,
                fetched_from="local_upload",
            )
            db.add(paper)
            db.commit()
            db.refresh(paper)
            ...
            analysis = PaperAnalysis(
                ...
                strengths=_serialize_list_for_db(strengths),
                weaknesses=_serialize_list_for_db(weaknesses),
                gaps=_serialize_list_for_db(gaps),
                semantic_patterns=_serialize_list_for_db(keywords),
            )
            db.add(analysis)
            db.commit()
            db.refresh(analysis)
            ...
            papers_with_reviews.append({
                ...
                "analysis": {
                    ...
                    "gaps": gaps,
                    "semantic_patterns": keywords,
                },
            })
        except Exception as e:
            db.rollback()
            console.log(f"[red]Failed saving local analysis: {e}[/red]")

    return JSONResponse(
        content={
            "count": len(papers_with_reviews),
            "papers_with_reviews": papers_with_reviews,
            "message": "✅ Local analyses saved successfully.",
        }
    )

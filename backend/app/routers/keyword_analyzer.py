...
# ====================================================
# Endpoint 4️⃣: Manually Store Keywords for a Project (Simplified + raw_query)
# ====================================================
...
@router.post("/manual-store", response_model=EnhancedKeywordFetchResponse)
def manual_store_keywords(
    data: ManualKeywordRequest,
    db: Session = Depends(get_db),
):
    """
    Manually add or update keywords and raw_query for a project.
    This does NOT handle summaries or files.
    """
    project = db.query(Project).filter(Project.project_id == data.project_id).first()
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")

    # ✅ Update project name and raw_query if provided
    if data.project_name:
        project.project_name = data.project_name
    if data.raw_query:
        project.raw_query = data.raw_query
    ...
    for k in data.keywords:
        ...
        if key and key not in seen:
            seen.add(key)
            deduped_keywords.append(k.strip())

    # ✅ Build expanded_query JSON structure (keywords only)
    expanded_data = {
        "keywords": deduped_keywords
    }

    project.expanded_query = json.dumps(expanded_data, ensure_ascii=False, indent=2)
    project.file_type = "manual"
    ...
    return EnhancedKeywordFetchResponse(
        ...
        keywords=deduped_keywords,
        summaries=None,
        message="✅ Manual keywords and raw query stored successfully."
    )
